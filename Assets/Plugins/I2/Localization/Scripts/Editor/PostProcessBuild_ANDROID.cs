...
namespace I2.Loc
{
...
#endif
    {
        ...
        public static void PostProcessAndroid(BuildTarget buildTarget, string pathToBuiltProject)
        {
            ...
            string hebrewShortCode = "he";
            string indonesiaShortCode = "id";
            ...
            foreach (var code in langCodes)
            {
                ...
                if (fixedCode == hebrewShortCode)
                {
                    fixedCode = GoogleLanguages.GetGoogleLanguageCode(fixedCode);
                }

                if (fixedCode == indonesiaShortCode)
                {
                    fixedCode = GoogleLanguages.GetGoogleLanguageCode(fixedCode);
                }

                Debug.Log($"fixedCode: {fixedCode}");
